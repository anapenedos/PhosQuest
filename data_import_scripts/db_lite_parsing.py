...
kin_sub_path = os.path.join('db_lite_tables', 'PhosphoSitePlus',
...
reg_sites_source_df = pd.DataFrame(
    rsdb_tbl[3:])  # Transform db list object to dataframe from row 4
reg_sites_headers = reg_sites_source_df.iloc[0]  # Define first row as header
reg_sites_source_df = reg_sites_source_df[
                      1:]  # Define data as table less the header row
...
bindingDB_source_df.rename(columns={
    "Target_Source_Organism_According_to_Curator_or_DataSource": "ORGANISM"},
                           inplace=True)
...
kin_sub_human = kin_sub_source_df[
    (kin_sub_source_df.KIN_ORGANISM == "human") & (
                kin_sub_source_df.SUB_ORGANISM == "human")]

# Parse "Phosphorylation_site_dataset" db human entries and pass to variable
phos_sites_human = phos_sites_source_df[
    (phos_sites_source_df.ORGANISM == "human")]

# Parse "Disease-associated_sites" db human entries and pass to variable
dis_sites_human = dis_sites_source_df[
    (dis_sites_source_df.ORGANISM == "human")]

# Parse "Regulatory_sites" db human entries and pass to variable
reg_sites_human = reg_sites_source_df[
    (reg_sites_source_df.ORGANISM == "human")]

# Parse "BindingDB" inhibitor database human entries and pass to variable
bindingDB_human = bindingDB_source_df[
    (bindingDB_source_df.ORGANISM == "Homo sapiens")]
